  ...
  const triggerCast = useCallback(
    async (streamUrl: string, metadata: CastMetadata) => {
      if (typeof window === "undefined" || !(window as any).cast?.framework) {
        alert(
          "Chromecast is not supported or ready on this browser. Try Google Chrome or MS Edge.",
        );
        return;
      }
      ...
      try {
        ...
        if (session) {
          ...
          if (
            ...
            !metadata.subtitleUrl.startsWith("blob:")
          ) {
        ...
        if (err !== "cancel" && err?.code !== "cancel") {
          ...
          alert(
            `Cast session notice: ${typeof err === "string" ? err : err?.message || "Failed to connect to Cast device."}`,
          );
        }
      }
    },
